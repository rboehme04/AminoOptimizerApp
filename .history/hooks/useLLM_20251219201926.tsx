 ...
 * 1. process.env.HF_TOKEN (if available)
 * 2. process.env.EXPO_PUBLIC_HF_TOKEN (Expo client-side)
 * 3. Constants.expoConfig?.extra?.HF_TOKEN (from app.json)
 * @returns API key or null if not set
 */
const getHuggingFaceApiKey = (): string | null => {
  // Try process.env first (works in Node.js/server contexts)
  const apiKey =
    process.env.HF_TOKEN ||
    process.env.EXPO_PUBLIC_HF_TOKEN ||
    Constants.expoConfig?.extra?.HF_TOKEN ||
    null;
  return apiKey || null;
};
 ...
 * 3. Add HF_TOKEN to your .env file or environment variables
 *
...
export const useLLM = (): UseLLMReturn => {
  ...
  const callLLM = useCallback(async (prompt: string, systemPrompt?: string) => {
    ...
    try {
      ...
      if (!apiKey) {
        throw new Error(
          "Hugging Face API key not found. Please set HF_TOKEN in your environment variables."
        );
      }
