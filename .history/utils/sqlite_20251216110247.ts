...
export const insertRecipe = async (
  ...
  const result = await db.runAsync(
    "INSERT INTO recipes (title, instructions, ingredients_json, nutrition_json, created_at, is_favorite) VALUES (?, ?, ?, ?, ?, 0);",
    [
      recipe.title,
      recipe.instructions,
      ingredientsJson,
...
export const getAllRecipesOrderedByCreatedDesc =
  async (): Promise<RecipeRow[]> => {
    ...
    const rows = await db.getAllAsync<RecipeRow>(
      "SELECT id, title, instructions, ingredients_json, nutrition_json, created_at, is_favorite, is_optimized FROM recipes ORDER BY created_at DESC;"
    );
    return rows;
  };
...
export const getRecipeById = async (id: number): Promise<RecipeRow | null> => {
  const db = await dbPromise;
  const rows = await db.getAllAsync<RecipeRow>(
    "SELECT id, title, instructions, ingredients_json, nutrition_json, created_at, is_favorite, is_optimized FROM recipes WHERE id = ?;",
    [id]
  );
...
export const updateRecipe = async (
  ...
  await db.runAsync(
    "UPDATE recipes SET title = ?, instructions = ?, ingredients_json = ?, nutrition_json = ? WHERE id = ?;",
    [recipe.title, recipe.instructions, ingredientsJson, nutritionJson, id]
  );
};
