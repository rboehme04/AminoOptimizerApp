...
export default function OptimizerScreen() {
  ...
  // Parse recipe data for useLLM hook
  const recipeForLLM = recipeData
    ? {
  ...
  // Call useLLM hook with recommended Lebensmittel and recipe data
  const { prompt } = useLLM({
  ...
  useEffect(() => {
    if (recommendedLebensmittel.length > 0 && recipeForLLM) {
      console.log("LLM Prompt:", prompt);
    }
  }, [recommendedLebensmittel, recipeForLLM, prompt]);
