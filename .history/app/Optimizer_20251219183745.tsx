          ...
          if (limitingAAValue?.column) {
            ...
            try {
              const query = supabase
                ...
                .select("id, name, protein, " + orderColumn + (needsSum ? ", " + limitingAAValue.column.slice(1).join(", ") : ""))
              ...
              if (queryError) {
                ...
                if (needsSum && Array.isArray(limitingAAValue.column)) {
                  processedData = data
                    .map(item => {
                      const sum = limitingAAValue.column.reduce((acc, col) => {
                        const val = (item as unknown as Record<string, number | null>)[col];
                        return acc + (typeof val === "number" ? val : 0);
                      }, 0);
                      return { ...item, _sum: sum };
                    })
                    .sort((a, b) => (b as { _sum: number })._sum - (a as { _sum: number })._sum)
                    .slice(0, 50)
                    .map(({ _sum, ...rest }) => rest);
                }
              ...
              console.error("Error fetching recommended Lebensmittel:", queryErr);
            }
          }
        }
