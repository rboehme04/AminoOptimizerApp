...
export default function OptimizerFinalScreen() {
  ...
  const handleSaveRecipe = async () => {
    ...
    try {
      ...
      // Navigate back to the index page with back animation
      // Go back through the stack until we reach index
      const navigateBackToIndex = async () => {
        // Navigate back multiple times to reach index
        // Typical flow: Index -> Optimizer -> OptimizerFinal (2 steps back)
        router.back();
        // Use setTimeout to allow the first back animation to complete
        setTimeout(() => {
          if (router.canGoBack()) {
            router.back();
          }
