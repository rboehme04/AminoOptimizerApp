...
export default function OptimizerScreen() {
  ...
  // Hook to call LLM API
  const { response, isLoading, error: llmError, callLLM } = useLLM();
  ...
  // Log the prompt when recommendedLebensmittel are set and recipe data is available
  useEffect(() => {
    if (prompt) {
  ...
  useEffect(() => {
    const testHuggingFace = async () => {
      ...
      try {
        await callLLM(
          "Say 'Hello from Llama 3.1!' in one sentence.",
          "You are a helpful assistant."
        );
      } catch (err) {
        console.error("❌ Hugging Face test failed:", err);
      }
    };

    // Run test once when component mounts
    testHuggingFace();
  }, [callLLM]);

  // Log LLM response when it arrives
  useEffect(() => {
    if (response) {
      console.log("✅ Hugging Face API Response:", response);
    }
    if (llmError) {
      console.error("❌ Hugging Face API Error:", llmError);
    }
    if (isLoading) {
      console.log("⏳ Hugging Face API: Loading...");
    }
  }, [response, llmError, isLoading]);
