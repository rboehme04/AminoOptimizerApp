...
import { initDatabase, updateRecipe } from "@/utils/sqlite";
  ...
  const loadDraftData = useCallback(async () => {
    ...
    try {
      ...
      const mappedIngredients: Ingredient[] = draft.ingredients.map(ing => {
        let caloriesText = ing.calories || "0";
        // Ensure "kcal" is present if calories exist
        if (caloriesText && !caloriesText.toLowerCase().includes("kcal")) {
          caloriesText = `${caloriesText} kcal`;
        } else if (!caloriesText) {
          caloriesText = "0 kcal";
        }
        return {
          id: ing.id,
