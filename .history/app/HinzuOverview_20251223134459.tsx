...
import { consumeShouldClearSearchOnFocus } from "@/utils/navigationFlags";
...
export default function HinzuOverviewScreen() {
  ...
  const shouldClearSearchRef = React.useRef(false);
  ...
  // Clear search and dismiss keyboard when screen comes into focus
  // This happens when navigating back from HinzuLebDetail
  useFocusEffect(
    React.useCallback(() => {
      if (shouldClearSearchRef.current) {
        setSearchQuery("");
        setSearchResults([]);
