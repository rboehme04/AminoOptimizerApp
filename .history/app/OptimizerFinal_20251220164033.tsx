...
export default function OptimizerFinalScreen() {
  ...
  const handleSaveRecipe = async () => {
    ...
    try {
      ...
      // Pop 2 screens at once (OptimizerFinal -> Optimizer -> Index) to get a single animation
      if ('pop' in navigation && typeof navigation.pop === 'function') {
        (navigation as any).pop(2);
      } else {
        // Fallback: call router.back() twice synchronously to minimize animation delay
        router.back();
        // Use requestAnimationFrame to ensure both calls happen in the same frame
