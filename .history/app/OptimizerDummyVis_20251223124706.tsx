...
export default function OptimizerDummyVisScreen() {
  ...
  useEffect(() => {
    if (showContent && !hasUserToggledRef.current) {
      const timeoutId = setTimeout(() => {
        // Double-check that user hasn't toggled during the timeout
        if (!hasUserToggledRef.current) {
          setAnimationDuration(700); // 700ms for automatic switch
  ...
  // Handle manual toggle - reset to manual duration
  ...
  const beforeData: AminoAcidData[] = [
    ...
    { name: "Met+\nCys", usable: limitingASBefore, unusable: 51 },
    { name: "Phe+\nTyr", usable: limitingASBefore, unusable: 120 },
          ...
          <View style={styles.contentContainer}>
            <View style={styles.aminoOptBarChartContainer}>
              ...
              <LeftRightToggle
                ...
                // inactiveColor with contrastchecker selected so that the contrast backgroundDarkElevated is right
