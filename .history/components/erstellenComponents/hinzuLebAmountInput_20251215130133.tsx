...
const HinzuLebAmountInput = ({
  ...
  const [selectedLabel, setSelectedLabel] = React.useState(dropdownItems[0]);
  ...
  const measureDropdownPosition = React.useCallback(() => {
    if (dropDownInputRef.current && containerRef.current) {
      ...
      // Also measure position in window for Modal positioning
      dropDownInputRef.current.measureInWindow((x, y, width, height) => {
        setWindowPosition({
          top: y + height,
          left: x,
...
export default HinzuLebAmountInput;
