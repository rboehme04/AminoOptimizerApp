...
const HinzuRezAmountInput = ({
  ...
  const handleTextChange = (text: string) => {
    // allow digits plus a single decimal separator (dot or comma)
    let sanitized = text.replace(",", ".").replace(/[^0-9.]/g, "");

    // ensure only one dot by keeping the first
    ...
    // allow a lone or trailing dot while the user is still typing
    if (sanitized === ".") {
      setValue("0.");
      return;
    }
