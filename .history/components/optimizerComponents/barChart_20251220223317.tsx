...
export default function StackedBarChart({
  ...
  const [interpolatedData, setInterpolatedData] = useState<AminoAcidData[]>(data);
  ...
  useEffect(() => {
    ...
    const listenerId = animationProgress.addListener(({ value }) => {
      const interpolated = data.map((item, index) => {
        ...
        return {
          name: item.name,
          usable:
            prevItem.usable + (item.usable - prevItem.usable) * value,
          unusable:
    ...
    // Store current data as previous for next animation
    prevDataRef.current = data;
