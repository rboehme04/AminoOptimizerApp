    ...
    <Svg width={width} height={height}>
      <G x={margin.left} y={margin.top}>
        ...
        {gridLevels
          ...
          .map(level => {
            const r = rScale(level);
            // Calculate where the grid circle intersects the left vertical line (x = 0)
            // Circle equation: (x - centerX)^2 + (y - centerY)^2 = r^2
            // At x = 0: centerX^2 + (y - centerY)^2 = r^2
            // (y - centerY)^2 = r^2 - centerX^2
            // y = centerY ± √(r^2 - centerX^2)
            // Use bottom intersection (beneath the line)
            const discriminant = r * r - centerX * centerX;
            let labelY = centerY;
            if (discriminant >= 0) {
              const offsetY = Math.sqrt(discriminant);
              labelY = centerY + offsetY; // Bottom intersection (beneath the line)
            }
            return (
              <SvgText
                key={`grid-label-${level}`}
                x={5} // Fixed position at the left edge
                y={labelY + 3} // Position slightly beneath the grid line
                fontSize="9"
                fill={Color.neutralTextOrTabGrey}
                textAnchor="start"
                opacity={0.6}
