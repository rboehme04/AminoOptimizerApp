...
export default function RadarChart({
  ...
  // Generate grid circles (0% to 250% in 50% increments)
  ...
  const labelPositions = displayData.map((d, i) => {
    const angle = startAngle + i * angleStep;
    const labelDistance = radius + 25; // Position labels outside the chart
  ...
  return (
    <Svg width={width} height={height}>
      <G x={margin.left} y={margin.top}>
        ...
        {/* Y-axis labels on the left side (at 0%, 50%, 100%, 150%, 200%) */}
        {[0, 50, 100, 150, 200].map(level => {
          const r = rScale(level);
          const labelY = centerY - r; // Top of the circle
          return (
            <SvgText
              key={`label-${level}`}
              x={-5}
        ...
        <Polygon
          points={pointsToPath(usablePoints)}
          fill={Color.success70}
        />

        {/* Reference circle for limitingAS */}
        <Circle
          cx={centerX}
          cy={centerY}
          r={rScale(animatedLimitingAS)}
          fill="none"
          stroke="white"
          strokeWidth={1.5}
        />
        ...
        <SvgText
          x={centerX + rScale(animatedLimitingAS) + 5}
          y={centerY + 5}
          fontSize="15"
          fill="white"
          textAnchor="start"
