...
export default function StackedBarChart({
  ...
  const currentAnimationDurationRef = useRef<number>(animationDuration);
  const isAnimatingRef = useRef<boolean>(false);
  const animationListenerRef = useRef<number | null>(null);
  ...
  const currentLimitingASDurationRef = useRef<number>(animationDuration);
  const isLimitingASAnimatingRef = useRef<boolean>(false);
  const limitingASListenerRef = useRef<number | null>(null);
  ...
  useEffect(() => {
    ...
    if (isAnimatingRef.current && animationListenerRef.current !== null) {
      animationProgress.stopAnimation();
      animationProgress.removeListener(animationListenerRef.current);
    ...
    // Update the duration ref when starting a new animation
    currentAnimationDurationRef.current = animationDuration;
    ...
    // Start animation using the duration from the ref
    Animated.timing(animationProgress, {
      toValue: 1,
      duration: currentAnimationDurationRef.current,
    ...
    return () => {
      if (animationListenerRef.current !== null) {
        animationProgress.removeListener(animationListenerRef.current);
        animationListenerRef.current = null;
      }
      isAnimatingRef.current = false;
  ...
  useEffect(() => {
    if (prevLimitingASRef.current !== limitingAS) {
      // If already animating, stop the current animation first
      if (isLimitingASAnimatingRef.current && limitingASListenerRef.current !== null) {
        limitingASAnimation.stopAnimation();
        limitingASAnimation.removeListener(limitingASListenerRef.current);
        isLimitingASAnimatingRef.current = false;
        limitingASListenerRef.current = null;
      }

      // Update the duration ref when starting a new animation
      currentLimitingASDurationRef.current = animationDuration;

      // Set the animation value to start from the previous value
      limitingASAnimation.setValue(prevLimitingASRef.current);
      isLimitingASAnimatingRef.current = true;
      ...
      limitingASListenerRef.current = listenerId;

      // Start animation using the duration from the ref
      Animated.timing(limitingASAnimation, {
        toValue: limitingAS,
        duration: currentLimitingASDurationRef.current,
        easing: Easing.out(Easing.cubic),
        useNativeDriver: false,
      }).start(() => {
      ...
      return () => {
        if (limitingASListenerRef.current !== null) {
          limitingASAnimation.removeListener(limitingASListenerRef.current);
          limitingASListenerRef.current = null;
        }
        isLimitingASAnimatingRef.current = false;
