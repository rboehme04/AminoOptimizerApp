      ...
      <G x={margin.left} y={margin.top}>
        ...
        {gridLevels
          ...
          .map((level, index) => {
            ...
            // Use a fixed x position for all labels (based on the leftmost point of the chart)
            const fixedX = centerX - radius + 5; // Fixed x position near the left edge
            // Add increasing y offset to prevent overlap (each label gets more offset)
            const yOffset = -(-5 + index * 15); // Base offset + increasing spacing
            return (
              <SvgText
                key={`grid-label-${level}`}
                x={fixedX} // Fixed x position so labels align vertically
