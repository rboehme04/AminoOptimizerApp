  ...
  const handleToggleFavorite = React.useCallback(async () => {
    ...
    try {
      if (isFavorite) {
        ...
        // Load full recipe data from database to get ingredients
        ...
        // Extract ingredients summary from recipe
        let ingredientsSummary = "";
        try {
          const parsedIngredients = JSON.parse(recipeRow.ingredients_json) as Array<{
            title?: string;
          }>;
          ...
          console.error("Error parsing ingredients for favorite", error);
        }

        // Create favorite item with full data
        const favoriteItem: FavoriteRecipeItem = {
          id: recipeId,
          title: recipeRow.title || title,
