...
const LeftRightToggle = ({
  ...
  const getLineStyle = () => {
    if (activeSide === "left" && leftTextLayout && leftContainerLayout) {
      ...
      const lineWidth = textWidth * 1.2; // 10% longer on each side (20% total)
      const lineLeft = absoluteX - textWidth * 0.1; // Shift left by 10% of text width
      return {
        width: lineWidth,
        left: lineLeft,
      };
    }
    if (activeSide === "right" && rightTextLayout && rightContainerLayout) {
