...
const mapRecipeRowToItem = (row: RecipeRow): RecipeItem => {
  ...
  if (row.nutrition_json) {
    try {
      const nutrition = JSON.parse(row.nutrition_json) as Record<string, number>;
...
// Component to calculate and display macros for a recipe row
const RecipeRowWithMacros = ({
  ...
  useEffect(() => {
    // Calculate macros asynchronously
    const calculateMacros = async () => {
      try {
        ...
        console.error("Error calculating macros for recipe", error);
      }
    };
