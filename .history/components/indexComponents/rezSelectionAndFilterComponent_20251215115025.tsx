...
const RezSelectionAndFilterComponent = (
  ...
  // Respect an explicitly provided activeSide even on the optimizer home,
  // otherwise fall back to the default "Rezept" for optimizer or "Lebensmittel" as a safe default.
  const effectiveActiveSide =
    props.activeSide ?? (isOptimizerHome ? "Rezept" : "Lebensmittel");
  ...
  // Use search results if provided, otherwise use the normal selection
  ...
  const hasNoFavorites =
    !searchResults && selection === "favoriten" && favoriteItems.length === 0;
