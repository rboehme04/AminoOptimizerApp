...
export default function SettingsCategoryRowComponent({
  ...
  placeholder,
  measuringUnit = "",
  isCustomDetailScreen = false,
}: {
  label: string;
  placeholder?: string;
  measuringUnit?: string;
  isCustomDetailScreen?: boolean;
}) {
  const router = useRouter();

  const handlePress = () => {
    const pathname = isCustomDetailScreen
      ? "/SettingsCustomDetailScreen"
      : "/SettingsDetailScreen";
    router.push({
      pathname,
      params: { label, placeholder, measuringUnit },
    });
  };
